...
def apply_complex4(fr, fi, input):
    ...
    inp_real,inp_imag=torch.chunk(input,2,1) # 将input拆成实部和虚部 inp_real,inp_imag -> 32,1,1,256
    ret_real=(fr(inp_real) - fi(inp_imag)) # 对实部进行处理 ret_real.shape -> 32,2048,1,256
    ret_imag=(fr(inp_imag) + fi(inp_real)) # 对虚部进行处理 ret_imag.shape -> 32,2048,1,256
    ret=torch.cat((ret_real,ret_imag),1) # ret.shape -> 32,4096,1,256
    return ret
