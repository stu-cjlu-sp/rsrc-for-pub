...
def get_feature_module(layer_index,device=None):
    ...
    weight1=vgg.state_dict()
    weight1['0.weight'] = weight1['0.weight'].sum(1, keepdim=True)
    vgg[0] = nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1)
    vgg.load_state_dict(weight1)
    vgg.eval()
