...
def main():
    ...
    X_IQ=generate_IQ(X) 
    del X
    X_norm=norm(X_IQ)#IQ特征

    del X_IQ 
    X_norm=X_norm[:,0:2,:] 

    X_reIQ, Y_re = resizesignal(X_norm,Y) #数据增强
    # print(X_reIQ.shape)
    Y_re1=np.array(Y_re) #扩增后的标签改成array形
    # print(Y_re.shape)
    X_train,Y_train,X_test,Y_test=train_test_split(X_reIQ,Y_re1,test_ratio=0.2) 
    ...
    filepath = '/home/sp432zq/zq/buchongshiyan2/model/(guiyihuahou)ours+IQ+allResizesignal(6Block).h5' 
