...
SNR = -16:2:0;                              % snr range 
    
for n = 1 : length(SNR)
    
    disp(['SNR = ',sprintf('%+02d',SNR(n))])                       % 
    datasetCWD = ['test_dataset_impulsive_0.3\',num2str(SNR(n)),'db'];
    datasetCWD1 = ['test_dataset_noise_impulsive_0.3\',num2str(SNR(n)),'db'];
        ...
        switch waveform                                                     
            case 'BPSK'
                ...
                for idx = 1:fps
                    ...
                    signal_power = sum(abs(wav).^2) / length(wav);  
                    noise_power = signal_power / snr_linear; 

                    p1 = 0.3; 
                    sigma1 = sqrt(noise_power * 4); 
                    sigma2 = sqrt(noise_power);   
                    b = binornd(1, p1, 1, length(wav));
                    g1 = normrnd(0, sigma1, 1, length(wav));
                    g2 = normrnd(0, sigma2, 1, length(wav));
                   
                    u = b .* g1 + g2;
                                
                    g1_snr = 10 * log10(sum(abs(wav).^2) / sum(abs(g1).^2));
                    fprintf('Impulsive SNR: %.2f dB\n', g1_snr);
                    g2_snr = 10 * log10(sum(abs(wav).^2) / sum(abs(g2).^2));
                    fprintf('Gussian SNR: %.2f dB\n', g2_snr);
                    u_snr = 10 * log10(sum(abs(wav).^2) / sum(abs(u).^2));
