...
A = 1;                                       % amplitude 
waveforms = {'BPSK','Costas','Frank','LFM','P1','P2','P3','P4','T1','T2','T3','T4'}; %13 LPI waveform codes 
...
imgSize = 256;                               
%% initial channels
atenuationset = -10:2:0; 
delayset = (1:50:500)*1e-9; 
atenuations = [0,atenuationset(randperm(length(atenuationset),3))]; 
delays = [0,delayset(randperm(length(delayset),3))]; 
multipathChannel = comm.RayleighChannel(...
    'SampleRate', fs, ...
    'PathDelays', delays, ...
    'AveragePathGains', atenuations, ...
    'MaximumDopplerShift', randi([5 500]));
...
for n = 1 : length(SNR)
    ...
    datasetCWD = ['test_dataset_mutipath2\',num2str(SNR(n)),'db'];
    datasetCWD1 = ['test_noise_dataset_mutipath2\',num2str(SNR(n)),'db'];
        ...
        switch waveform                                                       
            case 'BPSK'
                ...
                for idx = 1:fps
                    ...
                    multipathChannel.MaximumDopplerShift = randi([5 500]);
                    Bar = Lc(randi(3));
                    if Bar == 7
                    ...
                    wav1 = multipathChannel(wav');
                    wav1 = awgn(wav1,SNR(n),'measured');
            ...
            case 'Frank'
                ...
                for idx = 1:fps
                    release(multipathChannel);
                    atenuations = [0,atenuationset(randperm(length(atenuationset),3))]; % path gains
                    delays = [0,delayset(randperm(length(delayset),3))]; % path delays
                    multipathChannel.PathDelays = delays;
                    multipathChannel.AveragePathGains = atenuations;
                    multipathChannel.MaximumDopplerShift = randi([5 500]);
