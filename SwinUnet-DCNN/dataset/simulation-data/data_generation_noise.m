...
%% initial parameters configurations 初始参数配置
fs = 100e6;                                  
A = 1;                                       
waveforms = {'BPSK','Costas','Frank','LFM','P1','P2','P3','P4','T1','T2','T3','T4'}; %13 LPI waveform codes 13种LPI雷达波形
...
for n = 1 : length(SNR)
    ...
    datasetCWD = ['train_dataset\',num2str(SNR(n)),'db'];
    datasetCWD1 = ['train_noise_dataset\',num2str(SNR(n)),'db'];

    for i = 1 : length(waveforms)
    % create the folders for dataset storage 创建用于数据集存储的文件夹
     mkdir(fullfile(datasetCWD,waveforms{i}));                     % 在train\CWDpath5\-10db路径下继续创建文件夹
     mkdir(fullfile(datasetCWD1,waveforms{i}));                    % 在train\CWDpath5\-10db路径下继续创建文件夹
    end

    for k = 1 : length(waveforms)
        waveform = waveforms{k};
        switch waveform                                            % 通过switch case语句判断波形           
            case 'BPSK'
